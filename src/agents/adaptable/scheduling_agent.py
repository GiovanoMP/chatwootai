...
class SchedulingAgent(AdaptableAgent):
    ...
    def cancel_appointment(self, appointment_id: str, reason: str = None) -> Dict[str, Any]:
        ...
        if self.erp_client:
            ...
            if reason:
                appointment_data["cancel_reason"] = reason
            
            return self.erp_client.update_appointment(appointment_id, appointment_data)
        
        return {"error": "Cliente ERP não disponível"}
    
    def get_customer_appointments(self, customer_id: str, limit: int = 5) -> List[Dict[str, Any]]:
        """
        ...
        """
        if self.erp_client:
            return self.erp_client.get_customer_appointments(customer_id, limit)

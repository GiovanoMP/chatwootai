...
class KnowledgeManager:
    ...
    def store_knowledge(
        ...
        try:
            ...
            # Armazenar no Redis
            serialized_data = serialize_json(knowledge_item.to_dict())
            ttl = Config.KNOWLEDGE_CACHE_TTL
            ...
            success = redis_manager.set(cache_key, serialized_data, ttl)
            ...
            if success:
                ...
                existing_ids = redis_manager.get(topic_index_key)
                if existing_ids:
                    ids_list = deserialize_json(existing_ids)
                else:
                    ids_list = []
                
                if knowledge_item.id not in ids_list:
                    ids_list.append(knowledge_item.id)
                    redis_manager.set(topic_index_key, serialize_json(ids_list), ttl)
    ...
    def _retrieve_from_redis(self, account_id: str, topic: str, knowledge_id: str) -> Optional[KnowledgeItem]:
        """Recupera conhecimento do Redis"""
        try:
            cache_key = get_knowledge_cache_key(account_id, topic, knowledge_id)
            cached_data = redis_manager.get(cache_key)
            
            if cached_data:
                knowledge_dict = deserialize_json(cached_data)

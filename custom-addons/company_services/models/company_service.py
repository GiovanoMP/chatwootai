...
class CompanyService(models.Model):
    ...
    # Informações da Empresa (computadas a partir de res.company)
    ...
    country = fields.Char('País', compute='_compute_company_data', store=True, readonly=False, tracking=True)
    
    # Informações de contato (computadas a partir de res.company)
    ...
    # Informações fiscais (computadas a partir de res.company)
    ...
    def action_update_company_data(self):
        ...
        self.ensure_one()
        # Força a recomputação dos campos computados
    ...
    def action_save_company_info(self):
        """Salva explicitamente as informações da empresa e atendimento no registro atual.
        
        Esta função garante que todos os dados da empresa e configurações de atendimento
        sejam persistidos corretamente, mesmo quando importados de res.company.
        Também cria um novo registro se não existir nenhum.
        """
        self.ensure_one()
        
        # Força a atualização dos dados da empresa
        self._compute_company_data()
        
        # Prepara todos os dados para salvar
        vals = {
            ...
            'mercadolivre_url': self.mercadolivre_url,
            ...
            'sunday_start_time': self.sunday_start_time,
            'sunday_end_time': self.sunday_end_time,
        }
        ...
        return {
            ...
            'params': {
                ...
                'message': 'As informações da empresa e configurações de atendimento foram salvas com sucesso.',
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    sunday_start_time = fields.Char('Início Domingo', default='08:00', tracking=True)
    sunday_end_time = fields.Char('Término Domingo', default='12:00', tracking=True)

    # Menções de Canais Digitais
    ...
    ], string='Tipo de Avaliação', default='numeric_1_10', tracking=True,
       help='Define o formato de avaliação que o agente solicitará ao cliente')
       
    # Solicitar justificativa da avaliação
    request_rating_reason = fields.Boolean('Solicitar justificativa da avaliação', default=False, tracking=True,
        help='Se ativado, o agente solicitará ao cliente que explique o motivo da nota dada')
    rating_reason_message = fields.Text('Mensagem de solicitação de justificativa', 
        default='Poderia nos dizer o motivo da sua avaliação? Isso nos ajudará a melhorar nossos serviços.',
        tracking=True)
        
    # Solicitar sugestões de melhoria
    request_improvement_suggestions = fields.Boolean('Solicitar sugestões de melhoria', default=False, tracking=True,
        help='Se ativado, o agente solicitará sugestões de melhoria ao cliente')
    improvement_suggestions_message = fields.Text('Mensagem de solicitação de sugestões', 
        default='Gostaria de deixar alguma sugestão para melhorarmos nossos serviços?',
        tracking=True)

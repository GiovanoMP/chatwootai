#!/usr/bin/env python
"""
Script para testar a resposta rápida do agente de atendimento ao cliente simplificado.
...
from crew_tests.agents.simple_customer_service import get_simple_response
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Teste de resposta rápida simplificada")
...
def main():
    ...
    if args.query:
        ...
        try:
            logger.info("Chamando get_simple_response")
            result = get_simple_response(args.query, args.account_id)
            logger.info("get_simple_response concluído com sucesso")
    ...
    print(f"Bem-vindo ao teste de resposta rápida simplificada para a conta {args.account_id}")
    ...
    while True:
        ...
        start_time = time.time()
        try:
            result = get_simple_response(query, args.account_id)
        except Exception as e:
            logger.error(f"Erro ao processar consulta: {e}", exc_info=True)
            result = f"Erro: {str(e)}"
        end_time = time.time()
    ...
    print("\nObrigado por usar o teste de resposta rápida simplificada!")

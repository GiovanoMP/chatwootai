...
class BusinessRules2SyncController(http.Controller):
    
    @http.route('/business_rules2/sync', type='json', auth='user')
    ...
    @http.route('/business_rules2/sync_metadata', type='json', auth='user')
    ...
    @http.route('/business_rules2/sync_scheduling_rules', type='json', auth='user')
    ...
    @http.route('/business_rules2/sync_support_documents', type='json', auth='user')
    ...
    def sync_company_metadata(self, business_rule_id, env=None):
        ...
        try:
            ...
            # Construir URL completa
            url = f"{webhook_url}/api/v1/company-metadata/sync"
    ...
    def _get_webhook_url(self, env):
        ...
        return env['ir.config_parameter'].sudo().get_param('business_rules2.webhook_url', 'http://localhost:8004')

    def _get_api_token(self, env):
        """Obter token de API das configurações"""
        return env['ir.config_parameter'].sudo().get_param('business_rules2.api_token', 'development-api-key')

    def _get_account_id(self, env):
        """Obter account_id das configurações"""
        return env['ir.config_parameter'].sudo().get_param('business_rules2.account_id', 'account_1')

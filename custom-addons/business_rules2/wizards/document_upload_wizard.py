...
class DocumentUploadWizard(models.TransientModel):
    _name = 'business.document.upload.wizard2'
    ...
    business_rule_id = fields.Many2one('business.rules2', string='Regra de Negócio', required=True)
    ...
    def action_upload(self):
        ...
        attachment = self.env['ir.attachment'].create({
            'name': self.file_name,
            'datas': self.file,
            'res_model': 'business.support.document2',
            'res_id': 0,  # Será atualizado após a criação do documento
        })
        
        # Criar documento de suporte
        document = self.env['business.support.document2'].create({
            'name': self.name,

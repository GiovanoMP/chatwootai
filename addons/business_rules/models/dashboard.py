...
class BusinessRulesDashboard(models.TransientModel):
    ...
    active_temporary_rules_count = fields.Integer(string='Regras Temporárias Ativas', compute='_compute_rules_count')
    
    active_permanent_rules = fields.Many2many('business.rule.item', string='Regras Permanentes Ativas', compute='_compute_active_rules')
    active_temporary_rules = fields.Many2many('business.temporary.rule', string='Regras Temporárias Ativas', compute='_compute_active_rules')

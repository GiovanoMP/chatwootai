...
class ClientMetadata:
    ...
    account_id: str  # Alterado de client_id para account_id
    client_name: str
    domain: str
    path: str
    available: bool = True
...
class DomainLoader:
    ...
    def list_available_clients(self, domain_name: str) -> List[str]:
        """
        Lista as contas disponíveis para um domínio específico.

        Args:
            domain_name: Nome do domínio

        Returns:
            List[str]: Lista de IDs de contas disponíveis
        """
        available_accounts = []
        ...
        try:
            ...
            for item in os.listdir(domain_dir):
                ...
                if os.path.isdir(account_dir):
                    # Verifica se é um diretório de conta (formato account_X) ou formato legado (client_X)
                    is_account_dir = item.startswith('account_')
                    is_legacy_dir = item.startswith('client_')

                    if is_account_dir or is_legacy_dir:
                        # Verifica se existe o arquivo config.yaml da conta
                        config_file = os.path.join(account_dir, CONFIG_FILE)
                        if os.path.exists(config_file):
                            available_accounts.append(item)
        except Exception as e:
            logger.error(f"Erro ao listar contas para o domínio {domain_name}: {str(e)}")
    ...
    def load_domain(self, domain_name: str, force_reload: bool = False) -> Optional[Dict[str, Any]]:
        ...
        # Na nova arquitetura, o arquivo config.yaml é opcional
        # Vamos verificar se existe, mas não é obrigatório
        domain_config = {}
        if os.path.exists(config_file):
            domain_config = self._load_yaml_file(config_file) or {}

        try:

            # Na nova arquitetura, podemos ter um domínio sem configuração central
            # Vamos verificar se existem arquivos de account_id
            account_files = []
            for item in os.listdir(domain_dir):
                if item.startswith('account_') and item.endswith('.yaml'):
                    account_files.append(item)

            if not domain_config and not account_files:
                logger.error(f"Nenhuma configuração encontrada para o domínio: {domain_name}")
                return None

            # Se não temos configuração central mas temos arquivos de account_id,
            # criamos uma configuração mínima
            if not domain_config and account_files:
                logger.info(f"Domínio {domain_name} sem configuração central, mas com {len(account_files)} contas")
                domain_config = {
                    "name": domain_name,
                    "version": "1.0",
                    "description": f"Domínio {domain_name} gerado automaticamente"
                }
    ...
    def get_client_info(self, domain_name: str, account_id: str) -> Optional[ClientMetadata]:
        """
        ...
        try:
            ...
            if not domain_info.available:
                logger.warning(f"Domínio não disponível para obter informações da conta: {domain_name}")
                return None

            # Verifica se o diretório do account_id existe
            account_dir = os.path.join(self.domains_dir, domain_name, account_id)

            # Se não encontrar com o formato account_id, tenta com o formato antigo client_id
            # para manter compatibilidade durante a transição
            if not os.path.exists(account_dir):
                # Tenta o formato antigo (client_id)
                legacy_dir = os.path.join(self.domains_dir, domain_name, f"client_{account_id.replace('account_', '')}")
                if os.path.exists(legacy_dir):
                    logger.warning(f"Usando diretório legado para compatibilidade: {legacy_dir}")
                    account_dir = legacy_dir
                else:
                    logger.warning(f"Diretório da conta não encontrado: {account_dir}")
                    return None

            # Verifica se existe o arquivo de configuração do cliente
            client_config_file = os.path.join(account_dir, CONFIG_FILE)
            ...
            if os.path.exists(client_config_file):
                ...
                if client_config and "metadata" in client_config:
                    client_name = client_config["metadata"].get("client_name", account_id)
            ...
            logger.error(f"Erro ao obter informações da conta {account_id} no domínio {domain_name}: {str(e)}")
            return None

    def load_client_config(self, domain_name: str, account_id: str, force_reload: bool = False) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        try:
            # Construímos um identificador único para o cache de configurações de contas
            cache_key = f"{domain_name}::{account_id}"

            # Se a conta já foi carregada e não é forçado o recarregamento, retorna do cache
            if cache_key in self.domains_cache and not force_reload:
                return self.domains_cache[cache_key]

            # Verifica se temos informações da conta
            client_info = self.get_client_info(domain_name, account_id)
            if not client_info:
                logger.warning(f"Conta não encontrada: {account_id} no domínio {domain_name}")
                return None
            ...
            # Adiciona metadados importantes da conta
            if "metadata" not in client_config:
                client_config["metadata"] = {}

            client_config["metadata"]["account_id"] = account_id
            ...
            logger.info(f"Configuração de conta carregada com sucesso: {account_id} no domínio {domain_name}")
            return client_config

        except Exception as e:
            logger.error(f"Erro ao carregar a configuração da conta {account_id} no domínio {domain_name}: {str(e)}")
            import traceback
            logger.debug(f"Traceback: {traceback.format_exc()}")
            return None

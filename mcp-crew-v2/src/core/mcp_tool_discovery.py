...
class MCPToolDiscovery:
    ...
    def _fetch_tools_from_mcp(self, mcp_name: str, mcp_config) -> List[ToolMetadata]:
        """Busca ferramentas diretamente do MCP ou via conector"""
        tools = []
        
        # Primeiro, tentar usar conector se disponível
        try:
            # Remover prefixo 'mcp-' se presente
            clean_mcp_name = mcp_name.replace('mcp-', '') if mcp_name.startswith('mcp-') else mcp_name
            
            # Obter conector para o MCP (instanciando a fábrica)
            factory = MCPConnectorFactory()
            connector = factory.get_connector(clean_mcp_name, mcp_config.__dict__)
            
            if connector:
                logger.info(f"🔌 Usando conector para {mcp_name}")
                
                if callable(connector):  # Para MCPAdapt que precisa ser usado em contexto
                    with connector() as adapter_tools:
                        # Converter ferramentas do adaptador para ToolMetadata
                        tools_data = MCPConnectorFactory.convert_adapter_tools_to_metadata(adapter_tools, mcp_name)
                        tools = [self._create_tool_metadata(tool_data) for tool_data in tools_data]
                else:  # Para adaptadores como MongoDBAdapter e ChatwootDynamicAdapter
                    # Converter ferramentas do adaptador para ToolMetadata
                    tools_data = MCPConnectorFactory.convert_adapter_tools_to_metadata(connector.tools, mcp_name)
                    tools = [self._create_tool_metadata(tool_data) for tool_data in tools_data]
                
                if tools:
                    logger.info(f"✅ Descobertas {len(tools)} ferramentas do {mcp_name} via conector")
                    return tools
        except Exception as e:
            logger.error(f"❌ Erro ao usar conector para {mcp_name}: {e}")
        
        # Se não conseguiu com conector, tenta o método atual
        try:
            logger.info(f"🔍 Tentando descoberta direta para {mcp_name}")
            ...
            if mcp_name == 'mcp-mongodb':
                ...
                # Estratégia genérica para outros MCPs
                tools = self._discover_generic_mcp_tools(mcp_config)
        except Exception as e:
            logger.error(f"❌ Erro ao descobrir ferramentas do {mcp_name} diretamente: {e}")
    ...
    def _parse_tools_response(self, tools_data: List[Dict], mcp_config) -> List[ToolMetadata]:
        ...
        for tool_data in tools_data:
            try:
                tool = self._create_tool_metadata(tool_data, mcp_config.name)
                tools.append(tool)
            except Exception as e:
    ...
    def _create_tool_metadata(self, tool_data: Dict[str, Any], mcp_source: str = None) -> ToolMetadata:
        """Cria um objeto ToolMetadata a partir de dados de ferramenta"""
        return ToolMetadata(
            name=tool_data.get('name', ''),
            description=tool_data.get('description', ''),
            parameters=tool_data.get('parameters', {}),
            mcp_source=tool_data.get('mcp_source', mcp_source),
            last_updated=time.time(),
            cache_key=get_tools_cache_key("*", mcp_source, tool_data.get('name', ''))
        )

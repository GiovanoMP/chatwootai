...
from .connectors.mongodb_adapter import MongoDBAdapter
from .connectors.chatwoot_dynamic_adapter import ChatwootDynamicAdapter
...
class MCPConnectorFactory:
    ...
    def get_connector(self, mcp_name: str, config: Dict[str, Any]) -> Optional[Any]:
        """
        Obtém um conector para o MCP especificado.
        
        Args:
            mcp_name: Nome do MCP (mongodb, chatwoot, redis, qdrant)
            config: Configuração do MCP
            ...
            if mcp_name_clean == "mongodb":
                ...
                try:
                    return ChatwootDynamicAdapter(
                        base_url=config.get('url', 'http://localhost:8004')
                    )
                except Exception as e:

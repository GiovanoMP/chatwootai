...
DEV_ADDONS_DIR="$PROJECT_ROOT/custom_addons"
ALT_DEV_ADDONS_DIR="$PROJECT_ROOT/custom-addons"
...
function show_help() {
    echo -e "\n${BOLD}Uso: $SCRIPT_NAME [módulo] [opções]${RESET}"
...
if [ -z "$CUSTOM_SOURCE" ]; then
    SOURCE_DIR="$DEV_ADDONS_DIR/$MODULE_NAME"
    # Verifica caminhos alternativos se o padrão não existir
    if [ ! -d "$SOURCE_DIR" ]; then
        ALT_SOURCE_DIR="$ALT_DEV_ADDONS_DIR/$MODULE_NAME"

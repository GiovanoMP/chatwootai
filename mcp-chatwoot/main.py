...
import httpx
# import redis  # Comentado para remover dependência
import json
import hmac
import hashlib
import logging
...
from src.common.config import (
    ...
    # REDIS_HOST, REDIS_PORT, REDIS_PASSWORD, REDIS_DB,  # Comentado para remover dependência
    MCP_CREW_URL, MCP_CREW_TOKEN
)

# Conexão com Redis - Comentado para remover dependência
'''
try:
...
'''
# Definindo variáveis para substituir o Redis
redis_client = None
redis_available = False
logger.info("Redis desabilitado - usando armazenamento em memória")
...
async def health_check():
    """Endpoint de health check simplificado que não depende de conexões externas.
    Verifica apenas se o serviço MCP-Chatwoot está funcionando.
    """
    health = {
        "status": "healthy",
        ...
        "version": "1.0.0",
        "redis": "disabled",  # Redis foi desabilitado
        "mongodb": "disabled",  # MongoDB foi desabilitado
        "config": {
            "chatwoot_url": CHATWOOT_BASE_URL is not None,
            "chatwoot_token": CHATWOOT_ACCESS_TOKEN is not None,
            "mcp_crew_url": MCP_CREW_URL is not None,
            "mcp_crew_token": MCP_CREW_TOKEN is not None
        }
    }
    
    # Verificar quantas ferramentas estão disponíveis
    health["tools_count"] = len(tools)
    
    # Sempre retornar status 200 OK, já que o serviço está em execução
...
async def chatwoot_webhook(webhook: ChatwootWebhook):
    ...
    if webhook.event == "message_created":
        ...
        try:
            ...
            # Armazenamento da mensagem em log (substituindo Redis)
            message_key = f"chatwoot:message:{account_id}:{conversation_id}:{datetime.now().timestamp()}"
            message_data = {
            ...
            # Log da mensagem em vez de armazenar no Redis
            logger.info(f"Mensagem recebida: {message_key}")
            logger.info(f"Conteúdo da mensagem: {json.dumps(message_data)}")
...
async def startup_event():
    ...
    # Verificar conexões
    # Redis foi desabilitado
    logger.info("Serviço iniciado sem Redis (desabilitado)")

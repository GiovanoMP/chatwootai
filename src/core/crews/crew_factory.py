...
class CrewFactory:
    ...
    def create_crew(self, crew_id: str, domain_name: str = None) -> GenericCrew:
        """
        Cria uma instância de GenericCrew a partir da configuração do domínio.
        ...
        try:
            ...
            if not domain_config:
                raise ConfigurationError(f"Domínio '{domain_name}' não encontrado ou configuração vazia")
            ...
            try:
                ...
                active_plugins = plugin_manager.initialize_plugins_for_domain(domain_name)
                logger.info(f"Plugins inicializados para domínio {domain_name}: {', '.join(active_plugins) if active_plugins else 'nenhum'}")
            except ImportError:
                logger.info(f"Sistema de plugins não disponível - continuando sem plugins para o domínio {domain_name}")
            except Exception as e:
                logger.warning(f"Erro ao inicializar plugins para domínio {domain_name}: {str(e)}")
    ...
    def get_crew_for_domain(self, crew_id: str, domain_name: str = None) -> GenericCrew:
        """
        Obtém uma crew para um domínio específico, criando-a se necessário.
        ...
        # Chave para o cache
        cache_key = f"{domain_name or 'active'}:{crew_id}"
        ...
        # Criar a crew
        return self.create_crew(crew_id, domain_name)

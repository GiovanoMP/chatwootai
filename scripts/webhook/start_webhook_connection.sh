...
start_webhook_server() {
    ...
    # Verificar se o arquivo webhook_server.py existe
    if [ ! -f "${PROJECT_DIR}/src/api/webhook_server.py" ]; then
        echo -e "${RED}Erro: Arquivo webhook_server.py não encontrado em ${PROJECT_DIR}/src/api/webhook_server.py${NC}"
        exit 1
    fi
